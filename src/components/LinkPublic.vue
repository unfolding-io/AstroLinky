<template>
    <div class="flex-col flex lg:flex-row gap-4 md:gap-20 items-center w-full">
        <div class="flex flex-col justify-center gap-6 w-full h-fit max-w-lg">
            ...
            <form @submit.prevent="handleSubmit" class="flex flex-col justify-center gap-10 w-full h-fit">
                ...
                <Button type="submit" :disabled="!pass || !isUnique || notAllowed"
        ...
        <div class="w-full max-w-3xl">
            <div v-if="active.status === 'success'"
                ...
                <div class="grid grid-cols-1 gap-2 h-fit flex-1 min-w-[260px]">
                    <a class="truncate text-slate-400 hover:text-teal-400" :href="`${baseUrl}/${active.slug}`"
                        target="_blank">{{ active.href }}</a>
                    <div class="grid grid-cols-1">
    
                        <useClipboard v-slot="{ copy, copied }" :source="`${baseUrl}/${active.slug}`">
                            <TooltipProvider>
                                <Tooltip :open="copied">
                                    <TooltipTrigger asChild>
                                        <button @click="copy()"
                                            ...
                                            <div class="mr-4 truncate">{{ baseUrl }}/{{ active.slug }}</div>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em"
                                                class="shrink-0 absolute right-2" viewBox="0 0 24 24">
                ...
                <div
                    ...
                    <img :src="`/qr?href=${encodeURIComponent(`${baseUrl}/${active.slug}`)}&name=${active.slug})`"
                        alt="QR code" class="w-full h-auto" />
                    <a :href="`/qr?href=${encodeURIComponent(`${baseUrl}/${active.slug}`)}&name=${active.slug}&download=1`"
                        target="_blank"
    ...
    <div class="grid absolute inset-0 bg-slate-700/80 text-slate-200 text-2xl" v-if="loading">
        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" class="absolute top-4 right-4"
            viewBox="0 0 24 24">
